...
struct biGraph {
    ...
    //=======create by Lei Xing ===============
    std::unordered_map<std::pair<uint32_t,uint32_t>, float, pair_hash> edge_weight;
    //=======create by Lei Xing ===============
    ...
    void read(const std::string & filePath, const std::string & order) {
        ...
        n1 = in.getUInt();//左侧节点数量
        n2 = in.getUInt();//右侧节点数量
        m = in.getUInt();//总边数

        edges.resize(m);//edge size 调整为m
        e1.resize(m);//e1 size 调整为m
        e2.resize(m);//e2 size 调整为m
        pU.resize(n1 + 5);//pU
        pV.resize(n2 + 5);
        ...
        for(uint32_t i = 0; i < m; i++) {
            uint32_t _u = in.getUInt();
            uint32_t _v = in.getUInt()-n1;
            float _w = in.getFloat();
            edges[i].u = _u;
            edges[i].v = _v;
            edge_weight.insert(std::make_pair(std::make_pair(_u,_v),_w));
        }
        //=======modify by Lei Xing ===============
