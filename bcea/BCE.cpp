...
// }
                ...
                bc.left_size=ls[0];
                bc.right_size=ls[1];
                //printf("res:%d:%d:%d ",totalMaximalCount, ls[0], ls[1]);
                for(uint32_t i = 0; i <= 1; i++) {
                    for(auto u: realR[i]) {
                        //printf("%u ", g->old_lables[i][u]);
                        bc.vertices[i].emplace_back(g->old_lables[i][u]);
                    }
                    for(uint32_t j = S[i].c; j < S[i].r && i == z; j++) {
                        //printf("%u ", g->old_lables[i][S[i][j]]);
                        bc.vertices[i].emplace_back(g->old_lables[i][S[i][j]]);
                    }
