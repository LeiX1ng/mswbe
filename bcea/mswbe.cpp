...
//=====created by Lei Xing=====
void MSWBE::read_graph_txt(biclique bc){
    map<ui, set<udp>> biG;
    n1=bc.left_size;
    n2=bc.right_size;
    n=n1+n2;
    ...
    switch (strategy) {
        ...
        case 4:{// normalized two hop neighbors based vertex order
            ...
            for(ui i = 0; i < ids.size(); i++){
                ...
                double norm_d = (double)vec.size()/mswbe_max(factor,1);
...
//#endif
    ...
    for(auto & g : groupsInL){
        ...
        if(R.size() + CR.size() >= right_thd){
            ...
            max_level = mswbe_max(max_level, R.size() + CR.size());
    ...
    for(int level = 1; level <= max_level; level++){
        while(!searchNodeQueue.empty()){
            ...
            if(maximality && head.R.size() >= right_thd)
            {
                ...
                vector<ui>resL,resR;
                for(auto l:head.L){
                    resL.emplace_back(tmp_oid[l]);
                }
                for(auto r:head.R){
                    resR.emplace_back(tmp_oid[r]);
                }
...
void MSWBE::print_biclique(vector<pair<vector<unsigned int>,vector<unsigned int>>>& callmswberesult,vector<ui> &L, vector<ui> &R,biclique& bc){
    vector<unsigned int>vetexL;
    vector<unsigned int>vetexR;
    for(auto e:L){
        vetexL.emplace_back(bc.indexMapleft[e]);
    }
    for(auto e:R){
        vetexR.emplace_back(bc.indexMapright[e]+5);
    }
    callmswberesult.emplace_back(make_pair(vetexL,vetexR));
/*
    for(auto e : L){
        cout << indexMapleft[e] << " ";
    }
    cout << "| ";
    for(auto e : R){
        cout << indexMapright[e] << " ";
    }
    cout << endl;
*/

}
void MSWBE::print_results(vector<pair<vector<unsigned int>,vector<unsigned int>>>& callmswberesult,biclique& bc){
    //cout << "Total # results : " << result_count << endl;

    for(auto e : result_bicliques){
        print_biclique(callmswberesult,e.first, e.second,bc);
    }
}
...
void removeDuplicates(vector<pair<vector<unsigned int>, vector<unsigned int>>> &pairs) {
    ...
    for (auto &pair: pairs) {
        ...
        secondVector.clear();
        secondVector.insert(secondVector.begin(), uniqueElements.begin(), uniqueElements.end());
        uniqueElements.clear();
    }
